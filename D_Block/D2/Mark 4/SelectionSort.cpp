...
auto calculateRemainingElements = [](const std::vector<int>& data) {
    return [&data](const std::vector<int>& acc) {
        return data | std::views::filter([&](const int& val) {
...
auto findMinElement = [](const std::vector<int>& acc) {
    return [&acc](auto remaining_elements) -> std::vector<int> {
        std::vector<int> copy_acc = acc;

        if (remaining_elements.begin() != remaining_elements.end()) {
